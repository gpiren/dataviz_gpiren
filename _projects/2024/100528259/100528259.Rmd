...
Second, after careful exploration, I noticed that there was something wrong with the representation of sports: some sports were not present in the 1960 Summer Olympic games, however they were displayed in the facets that demonstrate 2016 games. Already spotting an issue in the visualisation, I will try and make an alternative to it that is simpler and more meaningful.

Before I go around spoiling all the fun by revealing my ideas, let's proceed to data cleaning and adjusting section.
...
1. As the original plot's biggest weakness is the difficulty in tracking trends of individual sports over time, I will try to address this issue by using a slope chart as the alternative. A slope chart is useful when comparing trends over time, using two data points for each observation and a line that connects those two data points. 
...
3. I might change the title - or the subtitle- and come up with a striking one. I might also use a distinctive font that will contribute to originality of the visual.

4. Who does not love 8-bit stuff? Well, not me. The inspiration of my alternative plot will be 8-bit aesthetics. I do not see why not, I spent my whole childhood playing 'Track & Field'!
...
In the second attempt, which is an output subsequent to the class presentation, I'll omit some of the men's sports branches and include women's sports into my plot. But for now, let's move onto the first one.
...
Lastly, I received criticism regarding colourfulness of the legend and the space it occupies in the plot. I decided to not address the colourfulness; simply because I like it that way. The space issue was resolved when I moved to having only two facets and reduced the number of sports displayed. Now, I believe my plot has a better balance between the sizes of the plot and legend.

Time for self-criticism - What I do not like very much about this plot is the space in the upper area of women's sports facet. I was not able to find a way around this issue and make the second facet appear more occupied, simply becaus it happens due to the nature of data. Women's BMI values start at a low point and do not show dramatic increases that would fill the upper side of the facet. 
